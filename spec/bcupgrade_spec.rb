...
describe Bcupgrade do
  ...
  describe '#check_list' do
    before do
      output = "atom (!)\n1password\nactprinter\nalfred\n"
      allow(Bcupgrade::BrewCask).to receive(:brew_cask_list).and_return(output)
    end
  ...
  describe '#check_version' do
    context 'When raise error "Error: File /Users/***/*** is not a plain file"' do
      it 'returns "error"' do
        ...
        allow(Bcupgrade::BrewCask).to receive(:brew_cask_info).and_return(brew_cask_info_error)
    ...
    context 'When the latest version is installed,' do
      it 'returns "nil"' do
        ...
        allow(Bcupgrade::BrewCask).to receive(:brew_cask_info).and_return(output)
